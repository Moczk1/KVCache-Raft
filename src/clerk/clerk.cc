...
void Clerk::PutAppend(std::string key, std::string value, std::string op)
{
	...
	while (true)
	{
		...
		if (!ok || reply.err() == ErrWrongLeader)
		{
			std::print("{}:Clerk::PutAppend】原以为的leader：{}"
			           "请求失败，向新leader{}重试  ，操作：{}:\n",
			    GetTime(), server, server + 1, op.c_str());
			if (!ok)
			{
				std::print("{}:重试原因,rpc失敗\n", GetTime());
			}
			if (reply.err() == ErrWrongLeader)
			{
				std::print("{}:重試原因:非leader\n", GetTime());
			}
...
void Clerk::Init(std::string configFileName)
{
	...
	for (int i = 0; i < INT_MAX - 1; ++i)
	{
		...
		ipPortVt.emplace_back(nodeIp,
		    atoi(nodePortStr.c_str())); // 沒有atos方法，可以考慮自己实现
	}
	// 进行连接
