	...
	else
	{
		if (DEBUG)
		{
			std::string info = std::format(
			    ...
			    "-kvserver{}]WaitChanGetRaftApplyMessage<--Server {} , get "
			    "Command "
			    "<-- Index:{} , "
			    "ClientId {}, RequestId {}, Opreation {}, Key :{}, Value :{}",
			    m_id, m_id, raftIndex, op.ClientId, op.RequestId, op.Operation,
    ...
    : m_skipList(6), m_id(me), m_maxRaftState(maxraftstate)
{
	...
	std::vector<std::pair<std::string, short>> ipPort;
	...
	for (int i = 0; i < INT_MAX - 1; i++)
	{
		...
		ipPort.emplace_back(nodeIp, atoi(nodePortStr.c_str()));
	}
	...
	m_raftNode->init(servers, m_id, persister, applyChan);
