...
long long Persister::RaftStateSize() const { return m_raftStateSize; }
...
Persister::Persister(int me, Option opts) : m_raftStateSize(0) {
  ...
  std::fstream file(m_raftStateFileName, std::ios::out | std::ios::in);
  ...
  if (!fileOpenFlag) {
    std::print("{}{}:\t\tPersister file open error!\n", __FUNCTION__, __LINE__);
  }
