...
{
    service.init("127.0.0.1", "50001");
    std::cout << __LINE__ << std::endl;
    service.addPeer("127.0.0.1", "50002");
        ...
        while (true)
        {
            for (const auto &[addr, stub] : service.m_peers)
            {
                ...
                if (status.ok())
                {
                    ...
                    std::cout << "connection false, next try time:" << std::chrono::system_clock::now() + std::chrono::milliseconds(500) << std::endl;
                }
            }
